...
3. Create code/<survey_key>/analysis/run.do as the preferred Stata analysis. It should import CSV, filter Qualtrics metadata rows when ResponseId exists, write clean.csv, write summary.md and summary.tex, and export each figure as both PDF and PNG under slides/<survey_key>/inputs/.
4. Create code/<survey_key>/analysis/run.py modeled on code/repo_smoke_test/analysis/run.py as the guaranteed Python fallback. It should write the same outputs as the Stata script.
